...
IPC * create_ipc ( int max_channel, int max_msg_size ) {
	...
	for ( p = (volatile SHM_CHANNEL *) ipc_id->shm, i = 1; i <= max_channel; i++ ) {
		p->status = IDLE;
		p = (volatile SHM_CHANNEL *) ( (char *) p + size );
	}
	ipc_id->sem_id = create_sem ( ); //creation of semaphores
	if ( ipc_id->sem_id == 0 ) return NULL;
	return ipc_id;
}
...
int send ( IPC * ipc_id, void * data, int size, int channel ) {
	...
	sem_lock ( ipc_id->sem_id ); // locking semaphore
	while ( p->status !=  IDLE ) ;
	p->size = size;
	p->chan_no = ipc_id->channel;
	memcpy ( (void *) p->msg, data, size );
	p->status = WRITTEN;
	sem_unlock ( ipc_id->sem_id ); //unlocking semaphore
	return IPC_SUCCESS;
}
